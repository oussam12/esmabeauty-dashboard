...
const AB_COLORS = { black: "#000000", rosePoudre: "#F5E8E7", roseNude: "#F4E7E7" };
...
const LS_KEY = "asmabeauty-dashboard-jsx";
const fmtCurrency = (n)=> (n || 0).toLocaleString("fr-FR", { style: "currency", currency: "EUR" });
...
export default function AsmabeautyDashboard(){
  ...
  const { prevLabel, prevCA } = useMemo(()=>{
    ...
    const from = startOfMonth(prev).getTime(), to = endOfMonth(prev).getTime();
  ...
  const repartition = useMemo(()=>{
    ...
    return Array.from(map.entries()).map(([categorie, montant]) => ({ name:categorie, value:montant, pct:Math.round((montant/total)*100) }));
  }, [prestationsPeriode]);
  ...
  const tauxRecurrence = useMemo(()=>{
    ...
    let recurrentes=0, total=0;
    byClient.forEach(times=>{
      times.sort((a,b)=>a-b); total+=1;
      for(let i=1;i<times.length;i++){
        const diffDays = Math.abs(times[i] - times[i-1]) / (1000*60*60*24);
        if (diffDays>=21 && diffDays<=35){ recurrentes+=1; break; }
      }
    });
    return total? Math.round((recurrentes/total)*100) : 0;
  }, [data.prestations]);
  ...
  // Forms state
  const [formP, setFormP] = useState({ date:new Date().toISOString(), nom:"", prenom:"", adresse:"", email:"", telephone:"", categorie:CATEGORIES_PRESTATIONS[0], montant:"", commentaire:"" });
  const [formD, setFormD] = useState({ date:new Date().toISOString(), categorie:CATEGORIES_DEPENSES[0], montant:"", commentaire:"", variable:true });
  ...
  const deletePrestation = (id)=> setData(d=> ({ ...d, prestations: d.prestations.filter(p=> p.id!==id) }));
  const deleteDepense = (id)=> setData(d=> ({ ...d, depenses: d.depenses.filter(p=> p.id!==id) }));
  ...
  const exportCSV = ()=>{
    ...
    a.href = url; a.download = `asmabeauty_export_${new Date().toISOString().slice(0,10)}.csv`; a.click();
    URL.revokeObjectURL(url);
  };

  const PIE_COLORS = [ AB_COLORS.black, "#1f2937", AB_COLORS.roseNude, AB_COLORS.rosePoudre, "#9ca3af", "#d1d5db", "#e5e7eb" ];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="flex flex-wrap items-center gap-3 p-4">
          <div className="flex rounded-2xl border border-zinc-200 p-1">
            {["jour","mois","annee"].map(v=>(
              <button key={v} onClick={()=>setVue(v)} aria-selected={vue===v}
                className={`px-3 py-1.5 rounded-xl text-sm ${vue===v? "bg-black text-white":"hover:bg-zinc-50"}`}>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button onClick={()=> setDateRef(new Date())} className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 text-sm shadow-sm hover:bg-zinc-50">Aujourd'hui</button>
            <button onClick={()=> setDateRef(new Date(dateRef.getFullYear(), dateRef.getMonth() - (vue==="annee"?12:vue==="mois"?1:0), dateRef.getDate() - (vue==="jour"?1:0))) } className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 text-sm shadow-sm hover:bg-zinc-50">◀</button>
            <button onClick={()=> setDateRef(new Date(dateRef.getFullYear(), dateRef.getMonth() + (vue==="annee"?12:vue==="mois"?1:0), dateRef.getDate() + (vue==="jour"?1:0))) } className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-3 py-2 text-sm shadow-sm hover:bg-zinc-50">▶</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        {[
          {title:`CA ${vue==='mois'?'du mois':'de la période'}`, value:fmtCurrency(caTotal), extra: (prevCA!=null? `vs ${prevLabel}: ${Math.round(caDeltaPct)}%` : null)},
          {title:"Nombre de prestations", value:nbPrestations, extra:"Total enregistré(s)"},
          {title:"Panier moyen", value:fmtCurrency(panierMoyen), extra:"CA ÷ nb prestations"},
          {title:"Marge nette (période)", value:fmtCurrency(margeNette), extra:"Recettes – charges variables"},
        ].map((k, i)=>(
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={serieCA}>
                ...
                <Bar dataKey="montant" radius={[8,8,0,0]} fill={AB_COLORS.black} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(v, n, p)=> [fmtCurrency(Number(v)), p?.payload?.name]} />
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select className="w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm shadow-sm"
                  ...
                  {CATEGORIES_PRESTATIONS.map(c=> <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-[#F5E8E7] px-2.5 py-0.5 text-xs">{fmtCurrency(caTotal)}</span>
          </div>
                ...
                {prestationsPeriode.sort((a,b)=> new Date(b.date)-new Date(a.date)).map(p=> (
                  <tr key={p.id} className="border-t">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button onClick={()=> deletePrestation(p.id)} className="text-rose-600 hover:underline" title="Supprimer">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="inline-flex items-center rounded-full border border-zinc-200 bg-[#F4E7E7] px-2.5 py-0.5 text-xs">
              {fmtCurrency(depensesPeriode.reduce((s,d)=> s + d.montant, 0))}
            </span>
          </div>
          <div className="max-h-96 overflow-auto rounded-xl border border-zinc-200">
            <table className="w-full text-sm">
              ...
              <tbody>
                {depensesPeriode.sort((a,b)=> new Date(b.date)-new Date(a.date)).map(d=> (
                  <tr key={d.id} className="border-t">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button onClick={()=> deleteDepense(d.id)} className="text-rose-600 hover:underline" title="Supprimer">
                        <Trash2 className="inline h-4 w-4" />
                      </button>
                    </td>
                  </tr>
